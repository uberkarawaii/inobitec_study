...
- /tests/unit - программы для юнит-тестирования и CMakeLists.txt для добавления их в граф. каждая юнит-программа
  называется по той функции, которую она тестирует. набор программ зеркален - для С и С++ сторон 
...
прогнать только юнит-тесты
```
ctest --test-dir build/debug -R "parse_point_cpp|parse_point_c" --output-on-failure
```
тут --output-on-failure важен, т.к. при падении кейса внутри юнит-теста надо понимать, что именно упало
