...
- /tests - в корне run_case.cpp - выходной .exe сверяет итоговый код программы и подаваемый ему код на равенство. 
  и check_contains.cpp - берёт `file1 file2`, ищёт подстроку `file2` в `file1`.
- /tests/expect - ожидаемые выводы при тестах на нормальных данных
- /tests/input_data - входные данные для задач t2-t4
...
#### upd: через make (default-режим - debug)
- `make all` - соберутся t1-t2 на Си и С++
- `make format` - отформатируются все .c .cpp .h .hpp
- `make test` - прогонятся тесты на нормальные данные и на коды выходов для t1-t2
