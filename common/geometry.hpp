#pragma once

#include <expected>
#include <string_view>

#include "parse_codes.hpp"

// geom.hpp включаетс€ и в geom.cpp, и в main.cpp
// из geom.cpp будет сделана dll, так что при сборке geom.cpp его ф-ции будут помечены дл€ экспорта
// внесени€ в таблицу адресов (.lib)
// при сборке main же надо указать, что эта ф-ци€ будет вз€та из .dll, будет импортирована - линкер
// поставит ссылку на неЄ в .dll
// т.е. дл€ управлени€ поведением заголовочника - условна€ компил€ци€
// и чтобы не было проблем при статической линковке - пустой COMMON_API дл€ соотв. константы

// в условную компил€цию был добавлен случай linux - там этих флагов вообще не будет
#ifdef _WIN32

#ifdef COMMON_STATIC
#define COMMON_API

#elif defined(COMMON_EXPORTS)
#define COMMON_API __declspec(dllexport)

#else
#define COMMON_API __declspec(dllimport)
#endif

#else
#define COMMON_API
#endif

struct Point {
    double x;
    double y;
    double z;
    // дефолт. реализаци€ == (посимв. сравнение) то же , что и x == o.x && y == o.y && z == o.z
    // не€вно становитс€ inline - т.е. не будет конфликтов при include geometry.hpp в main.cpp и geometry.cpp
    bool operator==(const Point&) const = default;
};

// распознавание x y z
// возвращает либо Point, либо код ошибки (произойдЄт перва€ ошибка при движении справа налево)
COMMON_API std::expected<Point, number_error> parse_point(std::string_view s);
