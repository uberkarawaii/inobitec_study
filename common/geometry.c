...
int parse_point(char* str, struct Point* p) {
    ...
    while (i < 3 && *pointer != '\0') {
        ...
        // это будет если остановка не на пробеле/конце или в самом начале строки уже нечисловые данные, тогда end ==
        // pointer
    ...
    // (pointer в конце итерации указывает на начало след. числа. и если после третьего числа что-то ещё - это
    // отловится)
    if (i != 3 || *pointer != '\0')
        return 1;
