...
// полный quoting
// Один аргумент -> фрагмент командной строки Windows, который CRT ребёнка
// разберёт ровно в этот аргумент (MS Learn "Parsing C++ command-line arguments").
// Внутренняя " -> \"; ряд \ прямо перед " или перед концом удваивается, чтобы
...
std::string quote_arg(std::string_view arg) {
    ...
    for (char c : arg) {
        // экранированный слеш. если попался он, то пока только ++счётчик. т.к. важно, что после
        if (c == '\\') {
            ++backslashes;
            // если после \ кавычка, обычная, не делиминер.
            ...
            out += '"';
            // счёт подряд идущих бэкслешей сбрасывается
            backslashes = 0;
        } else {
            // не перед кавычкой парсер не будет есть по два слеша. поэтому без удвоения
    ...
    // но сами слеши удваиваются т.к. они перед кавычкой
...
// виндоус-специфичная ветка для создания дочернего процесса с переданными аргументами и сравнение кода
    ...
    //  логический блок для своевременного уничтожения in/out/err Обёрток
    {
        ...
        // строка вида "main.exe" arg1 arg2 ...; проходят через полный quoting - так не будет проблем от бэкслешей и
        // кавычек
