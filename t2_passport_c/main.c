...
int main() {
    ...
    if (code == INT_PARSE_OUT_OF_RANGE) {
        fprintf(stderr, " ол-во вершин не помещаетс€ в 32-битное целое. ѕолучено: %s\n", clean_s_num);
        free(s);
        free(s_num);
        return data;
    }
    ...
    if (N < 1) {
        fprintf(stderr, " ол-во вершин должно быть положительным. ѕолучено: %" PRId32 "\n", N);
        free(s);
        free(s_num);
        return usage;
    }
