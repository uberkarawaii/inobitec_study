...
void fc_line(const char* input, std::chars_format fmt, const char* name) {
    ...
    std::cout << std::format(
        ...
        ec_name(ec), v, v, fp_class(v), (v != kSentinel) ? "yes" : "no", ptr);
}
...
void show(const char* input) {
    ...
    std::cout << std::format(
        ...
        fp_class(sv), static_cast<long long>(end - input), end, errno == ERANGE ? "ERANGE" : "0");
...
int main() {
    const char* cases[] = {
        "42",   " +5",    "+5",     "5e2",   "5e+2", "5e",  "1e400", "1e-400",   "1e-320",
        "0x10", "0x1p1",  "1.8",    "1.8p1", "inf",  "nan", "abc",   "infinity", "INFINITY",
